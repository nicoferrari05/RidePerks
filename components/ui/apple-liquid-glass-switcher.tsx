...
// Liquid-glass theme switcher (light / dark / dim). Adapted from the
// 21st.dev "apple-liquid-glass-switcher": same API and icons, but the glass
// look is done in plain CSS (apple-liquid-glass-switcher.css) instead of an
// SVG displacement filter, which only renders in Chromium and was shipped as
// a large inline image. Labels are in Spanish for this site.
...
interface ThemeSwitcherProps {
  ...
  className?: string;
  /** Subset of options to show, e.g. ["light", "dark"] on the platform. */
  themes?: Theme[];
}
...
export function ThemeSwitcher({
  ...
  const options = themes
    ? themeOptions.filter((o) => themes.includes(o.value))
    : themeOptions;
  ...
  return (
    <fieldset
      ...
      data-active={String(options.findIndex((o) => o.value === activeValue) + 1)}
      style={{ "--sw-count": options.length } as React.CSSProperties}
    >
      <legend className="switcher__legend">Tema de la página</legend>
      {options.map((option) => (
